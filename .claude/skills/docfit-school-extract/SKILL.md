...
description: 整理学校论文 Word 模板与要求，清理说明/样例，建立 Registry 填写接口、正文与目录结构，并完成逐页复核；涉及模板提取或质检时必须使用。
---
...
所有输入及其角色，能取得整份文档 inventory，能随时回到全局结构；渐进披露路径必须由 Agent
选择。

## 结果定义

得到一份干净、可填写、可复用且同时满足以下条件的 Word 候选：
...
所有 DOCX 修改只通过 `mcp__docfit__docx_edit` 完成。输入保持只读；需要修改时，中间版本写入任务
`work/`，不得直接写入发布目录。如果源模板已经满足结果定义，可以直接选择该只读 snapshot，
不要为了调用 Tool 而制造无意义修改。
...
- 最终候选是已验证的只读源 snapshot，或来自最后一次成功原子编辑；所有判断回到过全局视野；
