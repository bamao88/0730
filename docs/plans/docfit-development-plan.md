...
- Next action: 当前计划无剩余开发项；如开始 M2 后核心转换优化或确定性预设样式收口，
  先按 06 第 6.6 节另建可执行计划并完成 O0 观测面；恢复 M3 时另建并批准评测计划
- Follow-on decision: 核心转换优化、确定性预设样式收口与 M3 Eval 分轨；预设样式只在
  Agent 完成语义和学校证据处理后、最终 candidate/视觉复核/validate 前由 Tool 介入，
  不进入 Knowledge 或 Agent prompt
- Blocked on: none
...
## 10. 后续确定性预设样式收口决策（仅设计记录，尚未实施）

用户已批准把“Tool 持有版本化预设样式，并在 Agent 完成语义与学校证据处理后做最终
确定性收口”加入后续计划。该决定不重开已经完成的 M1–M2，也不表示当前 Tool 已经
具备这项能力。它不是 Knowledge 扩展、学校样式库或新的 Agent 阶段；实现前必须另建
可执行计划，并按长期文档规则同步受影响的 00–06。

### 10.1 介入时点

预设样式只在主 Agent 已经完成以下工作后介入：识别内容语义，解释当前任务学校材料，
解决或显式保留证据冲突，把学校已确认值与批准的模板局部例外应用到工作文档，并把
已经绑定的语义角色、学校确认值、明确清除和受保护例外输出为结构化事实。Agent 不读取
预设注册表，也不选择具体预设值或逐项决定使用哪个兜底；Tool 在最后一步只对已经绑定
的角色做机械完整性比较，并按产品注册表的固定 allowlist 补齐缺失属性。

它是最终候选产生前的最后一次确定性写入，不是在 Agent 已经宣称完成、生成最终 Adobe
candidate、完成全页视觉复核或通过 `docx_validate` 后修改文档。必要的先后关系是：

```text
Agent 完成语义判断和任务专属样式应用
  -> Tool 仅对注册表 allowlist 中的缺失属性执行预设样式收口
  -> 产生新的最终候选 DOCX 与 document hash
  -> 为该候选生成 Adobe candidate
  -> Agent 复核该候选全部页面
  -> docx_validate 独立验证
  -> Agent 才能作出完成声明
```

这是一条保证证据仍绑定最终文档的必要先后约束，不是应用壳中的固定工作流。任何预设
样式写入都会使旧文档 hash、旧 `object_ref`、旧 render 和旧 validation 失去最终证明
资格；写入后必须重新取得当前候选所需的结构、渲染、视觉和验证证据。

### 10.2 归属与数据边界

- 预设样式注册表由 Tool 实现随产品版本化、只读发布，包含明确 schema、版本、digest、
  语义样式角色、属性值、继承和兜底链；具体物理目录与 schema 在实现 Preflight 中锁定。
- 它属于现有 Tool 的确定性内部数据，不进入 Knowledge、Skill、Agent prompt、学校材料、
  学校 profile 或跨任务学校数据库，也不构成第六类产品资产。
- 首选在现有 `docx_edit` 中增加一个边界明确的最终样式收口 action；不新增第六个公开
  Tool。若现有 Tool 契约无法安全表达，实施必须停止并重新审批，而不是在应用壳中增加
  隐藏的文档修改步骤。
- Agent 负责在调用前确定内容语义、当前任务学校规则、受保护区域和角色绑定，但不接收
  或挑选预设值。Tool 只比较已绑定角色的结构化样式事实与内部注册表，并识别允许补齐
  的缺失属性；它不从文字、位置、字体相似度或预设值反推标题、正文、模板槽位等语义
  角色，也不为未绑定对象主动选择一种角色。
- 应用壳只继续配置 SDK、权限和输入输出，不在 Agent 返回后自动修改 DOCX，也不根据
  缺失项选择样式。

### 10.3 合并优先级与禁止覆盖

预设样式只执行 `fill-missing`，不能把产品默认值提升为学校事实。属性优先级固定为：
...
学校材料未声明某项属性仍表示“学校要求未知”。Tool 可以为了得到稳定、可打开的候选
文档使用产品预设值补齐注册表固定 allowlist 中的缺失属性，但必须把来源记录为
`system_preset_fallback`，不能写成 `school_confirmed`，也不能据此宣称学校格式验收通过。

以下内容禁止被预设样式覆盖：

- 当前任务证据已经确认的学校属性和值，包括“明确清除”；
- 经确认需要保留的学校固定内容、签字区域、表单对象和局部版式例外；
- 已经识别并批准保留的行内语义，例如强调、上下标、公式和超链接；
- Tool 无法确认语义角色、目标范围或属性缺失状态的对象；
- 用户源文档中仅因外观存在、但尚未被 Agent 解释为目标语义的直接格式。

兜底链必须无循环并终止于完整的产品预设角色。未知角色、冲突输入、没有终点的兜底链、
角色没有产品 allowlist 或任何前置条件失配，都使该 Tool 调用失败且不发布输出；禁止
静默落到 Word `Normal`、相邻段落样式或由 AI 临时生成的值。

### 10.4 Tool 输入、结果与可追溯性

最终接口的精确字段由后续实现计划锁定，但至少必须满足：

- 输入绑定当前工作文档的 `document_sha256`、明确的语义角色/目标范围、已确认属性、
  明确清除和受保护例外；输入不携带 Agent 选择的预设值，缺失属性与可补齐范围由 Tool
  根据版本化注册表确定性计算；
- 输出写入不同路径，保持源文件与输入工作副本不变，并沿用 `docx_edit` 的
  all-or-nothing、`committed`、重新打开、效果检查和非目标内容保护语义；
- 结果逐角色、逐属性记录最终值来源、预设注册表版本/digest、使用的兜底链、兜底计数、
  未解决项、冲突和新文档 hash；不得记录论文正文或学校材料正文；
- 相同输入 hash、相同任务参数和相同注册表版本必须产生等价结果；对已经收口的文档
  再执行一次必须幂等，不得不断追加直接格式或重复样式定义；
- O0 观测面和扩展后的 `conversion-report.json` 只记录脱敏汇总，例如 action、注册表
  版本/digest、兜底角色/属性计数、未解决计数、committed、耗时和错误码，不记录具体
  正文、学校证据文本或完整样式载荷。

### 10.5 验证与完成语义

实现计划必须先建立以下普通产品门；它们不属于延期的 M3 Eval：

- 单元测试覆盖属性缺失、明确值、明确清除、继承、局部例外、循环兜底、未知角色、
  幂等性和注册表 digest；
- Tool 契约测试证明只补缺失属性、绝不覆盖学校确认值、错误时不发布、源文件不变且
  输出可重新打开；
- 集成测试证明预设收口发生在最终 Adobe candidate 之前，并且收口后的新 hash 会使旧
  render、visual review 和 validation 不能作为完成证据；
- `docx_validate` 分开报告“符合当前任务已确认学校规则”和“使用产品预设兜底”，不能
  把后者折叠为前者；最终回复必须披露兜底范围和仍未知的学校要求；
- 同一合成输入的前后比较达到：学校确认属性被覆盖次数为零，所有允许兜底的缺失属性
  要么确定性解析、要么显式失败，内容与非目标对象无回归。

### 10.6 实施前置与停止门

这项能力改变转换行为，不随 O1–O4 性能优化自动进入范围。开始实现前必须：

1. 先完成 06 第 6.6 节要求的 O0 隐私安全观测和 `conversion-report.json` 基线；
2. 单独批准实现计划，锁定受支持语义角色、预设属性集合、注册表版本策略、
   `docx_edit` action、报告字段和一个主要可量化目标；
3. 同批更新受影响的 00、01、02、04、05、06、README、Tool contract 和 active capsule；
4. 保持五个公开 Tool、未匹配工具默认拒绝、Subagent 无写权限、主 Agent 统一合并与
   发布、OfficeCLI/Adobe 固定职责、源 hash 完成门、当前 candidate 真实性和独立最终
   验证不变；主 Agent Bash/Write 的信任权限不在该优化中收紧。

如果实现需要在 Agent 完成声明后由应用壳偷偷改文档、让 Tool 自行判断论文语义、保存
学校 profile、把预设值注入 Knowledge/Skill，或绕过最终 Adobe candidate 与全页复核，
该方案必须停止并重新设计。
