...
> 状态：IN_PROGRESS（O0.0–O0.2 已完成，当前阶段 O0.3）
> 日期：2026-08-03
...
- Current phase: O0.3 直接 ID 关联、覆盖状态与指标聚合
...
Current slice: 只执行 O0.3；不提前实现 queue/SQLite 事件写入、保留策略或 UI。
Scope: 本计划第 7 节的直接 ID/hash/ref 关联、生命周期状态、coverage 维度与安全指标聚合。
Non-goals: O0.4–O0.7 的生产行为、O1–O4、M3、第二 Agent loop、远程 collector、正文持久化。
Acceptance: O0.3 成功标准逐项有交错 Subagent、冲突、缺桥、重复、乱序和未知指标 fixture。
Verification: focused unit/contract/integration + ruff + mypy + build/lock + base doctor；关联测试只处理 sanitized event fixture，不读取 SDK transcript、页面图片或论文正文，也不发起 Adobe 调用。
Execution: 主会话为 root owner并直接完成该有界串行阶段；如范围扩张再恢复独立 worker。
Stop gate: 任何关系必须依赖时间邻近、Tool 名称或固定流程猜测，或指标必须读取正文/图片/完整模型历史。
```
...
- 新增 `docfit observe [--port PORT]`：启动本地 Web，固定 loopback，默认 `PORT=0` 选择
  ...
  URL、参数、环境变量或日志；
- 为 `docfit convert` 增加 `--observation {auto,off}`。阶段开发期间默认 `off`，测试和手工
  验证显式传 `auto`；只有 O0.7 全门通过后才把默认改为 `auto`，并保留 `off` 用于基线和
  故障排查；
...
| O0.3 | IN_PROGRESS | 直接 ID 关联、覆盖状态和指标聚合 | O0.2 | 可信 Tool/Subagent 树模型与 coverage |
