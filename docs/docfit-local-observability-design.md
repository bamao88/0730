...
需要重新调查时，用户在认证页面发起 POST，由本地服务调用操作系统原生目录选择器，
显式挂载一个候选任务目录；浏览器不能提交任意路径字符串来绕过选择。挂载只在当前 Web
会话内有效，绝对路径只保留在服务端内存中的授权句柄里，不写入观测索引、cookie、URL、
日志或导出。无图形会话时 O0 不提供目录挂载捷径。验证规则是：
...
- 06 把它放在 M2 后优化轨道的 O0，且明确当前只完成设计、尚未实施；
