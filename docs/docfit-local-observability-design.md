...
O0.4 把 safe event 写入任务目录和仓库之外的私有 SQLite v2 索引。同步路径只做有界
`1024 events / 16 MiB` 优先级入队，后台单 writer 批量提交并在 ack 后更新 coverage；
P0 至少保留 128 个槽位，压力下先丢 P2、再丢 P1。存储执行单事件、单 run、数据库/WAL、
