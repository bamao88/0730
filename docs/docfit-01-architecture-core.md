...
学校模板准备进一步把适应性判断与确定性流程控制分开。应用选择一个有界局部工作项，一次
绑定当前裁图中所有可见顶层对象、必要子 run、图片和逐对象 Registry 候选；语义 Agent 只提交
该工作项的分类/修改意图并判断修改后局部结果。额外 context 是证据不足时的例外路径，不是
获取普通候选的必经步骤。应用负责 checkpoint 推进、有界重试、失败状态和精确版本绑定。
Agent 不接触区域游标、文档/区域版本引用或发布动作，也不写 mutation YAML/JSON、不调用
compiler、不管理 attempt 路径。

相邻同页工作项可以复用一个小而有界的 Claude Agent SDK 原生 client/session，以避免重复的
进程与上下文启动成本；应用在每个工作项之间显式重绑 Tool state，并要求 Agent 丢弃上一工作项
的对象、候选、图片和提交状态。跨页、达到上限或出现异常时立即轮换原生 session。应用只持有
这层领域调度与持久化运行指标，不保存或重放模型上下文，也不实现第二套会话运行时。

局部语义工作完成后，应用为精确候选版本组织独立视觉审查：按内部游标分批取得全部原生全页
PNG，视觉审查 Agent 为每页返回显式 clean/defect 结论；返回图片本身不能计作已审查。缺陷被
转换为有界单页修复工作项，任何编辑都会使旧 hash 的页面结论失效，并对新版本从第一页重查。
全页 clean 后由应用原子发布一份可填写 Word 和一份与其 hash 绑定的填写契约；任一缺失或
绑定不一致都不是成功交付。该编排是模板准备领域的有限状态约束，
仍通过 Claude Agent SDK 原生会话、custom MCP Tool 和 structured output 执行，不是第二套
通用 Agent runtime。
...
恢复优先使用 SDK 原生会话能力。DocFit 不维护自定义会话 checkpoint 图，也不在架构文档中
规定 SDK transcript 的持久化方式。模板准备落盘的当前文档 hash、对象区域位置和视觉判定只
是领域工作产物与幂等恢复依据，不保存或重放 Agent 上下文。

如果工具调用中途失败：

- Tool 返回明确错误且不覆盖源文件；
- 普通开放式任务由 Agent 决定重试、换工具、缩小范围或询问用户；模板准备的确定性重试次数、
  页面批次和终止状态由应用约束；
