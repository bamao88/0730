...
学校模板准备进一步把适应性判断与确定性流程控制分开。应用选择一个有界局部工作项并绑定
当前对象、图片和 Registry 候选；语义 Agent 只提交该工作项的分类/修改意图并判断修改后局部
结果。应用负责 checkpoint 推进、有界重试、失败状态和精确版本绑定。Agent 不接触区域游标、
文档/区域版本引用或发布动作，也不写 mutation YAML/JSON、不调用 compiler、不管理 attempt
路径。
...
全页 clean 后由应用自动发布，最终只交付一份 Word。该编排是模板准备领域的有限状态约束，
