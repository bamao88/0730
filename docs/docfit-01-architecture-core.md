...
`SKILL.md` 保留目标、输入/产物、边界、关键判断入口与完成条件；较长的冲突处理、分类、委派任务包、
...
Skill 的主要信息按四层披露：L0 `SKILL.md` 保存选择、边界和关键判断；L1 同目录
references 保存详细领域方法；L2 产品 Knowledge 保存跨学校通用概念；L3 当前任务
材料、冻结模板产物和 Tool 证据保存具体事实。机器可强制的不变量应下沉到 Tool、
应用壳和测试，不在 L0 重复成长清单。
