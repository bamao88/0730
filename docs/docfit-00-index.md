...
页面批次与图片载荷、收紧无效重试。该轨道已完成 O0.0–O0.2：平台无关骨架、SDK
transcript/report v2 隐私合同，以及入队前字段 allowlist projector；关联聚合、历史索引、
网站和 O0 总体验收尚未实现。它不能产生真实交付质量、MVP 或 M3 已通过的声明。
...
当前已完成设计与 O0.0–O0.2 的安全事件投影；事件持久化、历史索引和网站尚未实现。
