...
- Latest user intent: 保持已完成 M2 基线，按
  `docs/plans/docfit-o0-local-observability.md` 分阶段实施 O0；当前进入 O0.3，执行状态见
...
- Follow-on status: 06 第 6.6 节的核心转换优化边界与 O0 可执行计划已经批准；O0.0–O0.2
  已完成，当前执行 O0.3；
  不改变本 capsule 的 COMPLETED 状态，也不构成 M3 恢复或通过
- Approved follow-on design: `docs/docfit-local-observability-design.md` 定义薄壳内本地
  只读观测页；当前已有 privacy-safe 逐事件投影与 runtime 接线，无关联聚合、历史索引或网站实现
...
- No-touch scope: M4/M5、面向转换用户的 GUI/API/任务队列、学校规则持久化、通用
  Provider 抽象；O0.4–O0.7 必须继续按已批准计划顺序实施
