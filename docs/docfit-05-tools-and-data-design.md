...
hash；固定、填充、生成、重复、条件、人工和未决责任均有一致表达；生成机制没有被压成
缓存文字，重复区域没有把源示例数量冻结为实例基数；固定内容未被未经证据改写；学生源
内容清单中每个任务级 `source_item_id` 恰有一个
...
2. 绑定该 DOCX 精确 SHA-256 的槽位索引。来源证据、冲突、未决项、manual 区域、gap
   和验证摘要都是索引字段，不拆成额外交付文件。

槽位索引需要表达当前模板区域的责任：固定、填充、生成、重复、条件、人工或未决。
可自动处理区域至少具有任务内唯一 ID、冻结快照内的 locator、期望内容种类和基数；
首版内容种类只需要表达标量、段落流和复合内容，manual 是区域责任而不是伪内容类型。
生成区域保留生成关系，重复区域使用开放基数，条件区域保留适用条件；这是一组 Interface
语义，不是让 Skill 维护新的类型系统。locator 必须由至少一个 snapshot-bound 结构引用
及其前置指纹/上下文支持，并在合同检查时唯一命中；页码、bbox 或近似文字只能作为辅助
信号。索引不得承诺跨模板修改、跨任务或跨运行稳定。
