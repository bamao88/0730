...
Services API。仓库外凭据、SDK 4.2.0、路由、测试、Eval 和文档必须作为同一修正验证。
...
### 6.6 M2 后独立切片：核心转换优化（已记录，尚未实施）

这是一条位于 M2 完成之后的工程优化轨道，不重开 M2，也不恢复或替代 M3。当前只
批准把边界与顺序写入长期文档；实际实现应另建可执行计划。M2 完成提交作为不可变
比较基线，具体提交与历史运行数字记录在 active capsule，不写成长期行为合同。

开始条件全部必需：

1. 冻结并记录已经通过全量确定性门、doctor 与真实合成产品门的 M2 基线；
2. 先为 `conversion-report.json` 增加隐私安全的运行观测：总耗时、各 Tool 调用数/
   状态/耗时、解析与渲染缓存命中、Adobe API 调用与缓存命中、渲染/审查页数、图片
   输入字节、重试次数和首个失败来源；
...
1. **O0 观测面**：只增加上述任务级统计，不记录正文、学校材料正文、凭据或未经授权
   的绝对路径；
...
该切片的每次合入都必须证明：源文件不变；五个公开 Tool 和默认拒绝不变；OfficeCLI/
...
任务、跨任务持久缓存和正式性能平台；OCR/更多输入格式；精确版式模型或像素级自动
判定；高级 Word 对象编辑、Word 桌面兼容性；Eval 平台、replay 和学校数据库。
...
| M2 后优化切片 | 同一转换链路在既有安全门下减少可测量的重复工作 | 已证明的单项耗时、调用或载荷改善 | M3、MVP 或真实论文质量已通过 |
