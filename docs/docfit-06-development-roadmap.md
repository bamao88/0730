...
不建立第二 Agent loop，也不属于面向最终转换用户的 GUI 产品化。当前完成的是设计
记录，不是逐事件采集、历史索引或网站实现。
...
### 6.6 M2 后独立切片：本地观测与核心转换优化（设计已记录，尚未实施）

这是一条位于 M2 完成之后的工程优化轨道，不重开 M2，也不恢复或替代 M3。当前只
批准本地观测界面的目标设计、数据边界与优化顺序；实际采集、存储和网站实现应另建
可执行计划。M2 完成提交作为不可变比较基线，具体提交与历史运行数字记录在 active
capsule，不写成长期行为合同。
...
report、显式挂载、登录/session、Host/Origin/CORS/CSRF、路径/symlink 和资源预算必须通过；
