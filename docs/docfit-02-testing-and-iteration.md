...
- `SKILL.md` 只指向一个特殊论文部件索引，索引按当前文档中的目录、封面、声明、
  页眉页脚等部件引导读取 references；不按处理步骤逐一绑定 reference；
...
- `SKILL.md` 保留 L0 任务、输入、两个产物、通用规则和特殊部件索引；Tool 注册信息、
  参数、错误恢复和机器检查不进入 Skill。
...
| 归因 | 典型问题 | 修复位置 |
|---|---|---|
| Skill | 任务边界、两个产物、通用规则或特殊论文部件说明错误 | `.claude/skills/` |
