...
M2 后已批准先建设本地只读运行观测界面，当前已完成 O0.0–O0.3 的平台无关骨架、SDK
runtime privacy/report v2、字段级安全 projector，以及直接 ID 关联、覆盖维度和带来源指标；
持久化和网站尚未实现。它服务核心转换问题定位和性能比较，不是 M3 Eval 平台。并发 runner、实验平台、集中式
