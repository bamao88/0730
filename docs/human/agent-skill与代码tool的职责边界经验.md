...
### 4.3 固定后置地标与具名正文样例的区分

例如，“第X章（正文标题）”是通用可重复章的直接证据；“第一章 文献综述”和
“结论与展望”是复用同一正文样式体系的具名写作样例，不应形成独立正文接口；“参考文献”和
“附录”才可能是正文结构之外的固定后置地标。

它们在 Word 中可能都使用 Heading 样式，但语义处置不同。Agent 负责从局部证据选出一组通用正文
代表对象；代码负责保证最终只有一个完整的 `body.chapters`，并拒绝结构外正文接口和遗留具名样例。
...
- 正文不是一组互不相关的独立 slot；
- 最终只保留一组可复用的章标题、一级节标题、二级节标题和正文段落；
- 图、表、公式、列表等只在当前学校实际展示时各保留一个代表成员；
- 不因 Registry 存在某个字段就凭空制造对象；
- 不得删除尚未纳入代表结构的唯一对象类型；
- 文献综述、结论与展望等具名样例不能替代通用正文块，也不能形成第二套接口。

Agent 判断哪些连贯对象最能代表本校正文能力；代码验证结构是否唯一、最小成员是否齐全、每种成员
是否恰好一个，以及是否仍有结构外接口或具名样例残留。目录刷新和最终逐页检查只能在该门禁通过后开始。
